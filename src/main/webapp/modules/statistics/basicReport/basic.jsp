...
<head>
	...
	<script type="text/javascript">
		...
		$(function () {
		    $("div.menu-status li a").click(function () {
		        $("div.menu-status li").removeClass("on");
		        $(this).parent("li").addClass("on");
		        
		        var id = $(this).parent("li").attr('id');
		        if(id=='1'){
		        	$('#div_sbz').css('display','');
		        	$('#div_ylx').css('display','none');
		        	$('#div_yys').css('display','none');
		        }else if(id=='2'){
		        	$('#div_sbz').css('display','none');
		        	$('#div_ylx').css('display','');
		        	$('#div_yys').css('display','none');
		        }else if(id=='3'){
		        	$('#div_sbz').css('display','none');
	...
	<div class="main_bigbox">
		<div class="leftstatus">
			<div class="menu-status">
				<ul>
					<li class="on" id="1"><a href="###" class="sbz">申报中</a> </li>
					<li id="2"><a href="###" class="ylx">已立项</a> </li>
					<li id="3"><a href="###" class="yys">已验收</a> </li>
				</ul>
			</div>
		</div>
		...
		<div class="main_rightcontent" style="background-color:#F3F1F1!important;margin-left:90px; overflow:auto;" id="div_sbz">
			<div class="basic-search">
				<div class="basic-search-div">
					名称 <input type="text" id="" name="" size="20" />&nbsp;&nbsp;
					编号 <input type="text" id="" name="" size="20" />&nbsp;&nbsp;
					主持人 <input type="text" id="" name="" size="20" />&nbsp;&nbsp;
					主持单位 <input type="text" id="" name="" size="20" />&nbsp;&nbsp;
					<button type="button" class="button blue" style="width: 70px;">检索</button>
					<button type="button" class="button blue" style="width: 70px;">清空</button>
				</div>
			</div>
			...
			<table width="98%" align="center" bgcolor="#FFFFFF" style="border: solid thin #DED9D8" cellpadding="0" cellspacing="0">
				<tr height="40" bgcolor="#F7F7F6">
					...
					<td align="left" width="15%">&nbsp;&nbsp;<font color="blue">名称</font></td>
					<td align="left" width="10%">&nbsp;&nbsp;<font color="blue">资源库编号</font></td>
				...
				<c:forEach items="${sbzList}" var="zyk" varStatus="status">
					<c:if test="${zyk.status == 1}">
		...
		<div class="main_rightcontent" style="background-color:#F3F1F1!important;margin-left:90px; overflow:auto; display: none;" id="div_ylx">
			<div style="height: 10px;"></div>
			<table width="98%" align="center" bgcolor="#FFFFFF" style="border: solid thin #DED9D8" cellpadding="0" cellspacing="0">
				<tr height="40" bgcolor="#F7F7F6">
					<td align="left" width="2%">&nbsp;&nbsp;<font color="blue"></font></td>
				...
				<c:forEach items="${ylxList}" var="zyk" varStatus="status">
					<c:if test="${zyk.status == 2}">
						<tr height="40" <c:if test="${status.count%2==0}">bgcolor="#FCFCFC"</c:if>>
							<td align="center" style="border-top:1px solid #DED9D8;">${status.index + 1}</td>
							<td align="left" style="border-top:1px solid #DED9D8;">
								&nbsp;&nbsp;
								<font color="${zyk.sendColor}">
									<c:if test="${fn:length(zyk.fullname)>18}">${fn:substring(zyk.fullname,0,18)} ...</c:if>
									<c:if test="${fn:length(zyk.fullname)<=18}">${zyk.fullname }</c:if>
								</font>
							</td>
							<td align="center" style="border-top:1px solid #DED9D8;">
								<c:if test="${empty zyk.zykNum}">无</c:if>
							...
							<td align="left" style="border-top:1px solid #DED9D8;">
								&nbsp;&nbsp;
								<font color="${zyk.sendColor}">
									<c:if test="${fn:length(zyk.instituteInCharge)>18}">${fn:substring(zyk.instituteInCharge,0,18)}...</c:if>
									<c:if test="${fn:length(zyk.instituteInCharge)<=18}">${zyk.instituteInCharge }</c:if>
								</font>
							</td>
							<td align="center" style="border-top:1px solid #DED9D8;">&nbsp;&nbsp;${zyk.personInCharge}</td>
							<td align="center" style="border-top:1px solid #DED9D8;">&nbsp;&nbsp;${zyk.modifiedDate}</td>
		...
		<div class="main_rightcontent" style="background-color:#F3F1F1!important;margin-left:90px; overflow:auto; display: none;" id="div_yys">
			<div style="height: 10px;"></div>
			<table width="98%" align="center" bgcolor="#FFFFFF" style="border: solid thin #DED9D8" cellpadding="0" cellspacing="0">
				<tr height="40" bgcolor="#F7F7F6">
					<td align="left" width="2%">&nbsp;&nbsp;<font color="blue"></font></td>
				...
				<c:forEach items="${yysList}" var="zyk" varStatus="status">
					<c:if test="${zyk.status == 3}">
						<tr height="40" <c:if test="${status.count%2==0}">bgcolor="#FCFCFC"</c:if>>
							<td align="center" style="border-top:1px solid #DED9D8;">${status.index + 1}</td>
							<td align="left" style="border-top:1px solid #DED9D8;">
								&nbsp;&nbsp;
